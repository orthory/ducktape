...
fn staging_rejections_carry_the_files_prefix() {
    ...
    let err = f.putblob(&files::Authority::External { key: vec![0xaa], account: None }, 1, &[]).unwrap_err();
    assert!(err.starts_with("files: "), "empty chunk: {err}");
    let big = vec![0u8; CHUNK_SIZE as usize + 1];
    let err = f.putblob(&files::Authority::External { key: vec![0xaa], account: None }, 1, &big).unwrap_err();
    assert!(err.starts_with("files: "), "oversized chunk: {err}");
    f.set_staging_quota_for_tests(4);
    let err = f.putblob(&files::Authority::External { key: vec![0xaa], account: None }, 1, b"hello").unwrap_err();
