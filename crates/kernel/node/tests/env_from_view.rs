...
fn env_reflects_agreed_view_height_time_and_real_origin() {
    futures::executor::block_on(async {
        ...
        for node in nodes.iter_mut() {
            node.submit(b"alice", 0, op(b"first")).await.expect("submit");
            drain_to_fixpoint(node).await;
        }

        // PHASE 2: a LATER op (submitter "bob") -> agreed view 1 (strictly higher).
        for node in nodes.iter_mut() {
            node.submit(b"bob", 0, op(b"second")).await.expect("submit");
            drain_to_fixpoint(node).await;
        }
        ...
        // origin is the REAL submitter (frame.origin bytes), not empty.
        assert_eq!(s0.origin, b"alice", "root origin is the real submitter");
        assert_eq!(s1.origin, b"bob");
