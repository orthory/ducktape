...
impl IndexStore {
    ...
    /// watermark across all modules. every module advances on every applied
    /// block, so the max only differs per module when a database was wiped
    /// or added — exactly the modules [`IndexStore::rebuild_module`] repairs.
