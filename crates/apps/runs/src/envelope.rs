...
Allowed reply block kinds are paragraph, heading, and code. heading is rendered as a paragraph in Ducktape chat. code may include an optional "lang". Actions are optional and must use only actions allowed by the agent registry. Use the live ducktape_delegate and ducktape_delegations tools for peer calls. The terminal delegations field remains only for older runners that cannot call tools mid-run; it is shaped as {"agent_id":"<registered agent>","instruction":"...","skills":["<library skill name>"]}. Every call uses caller ∩ callee authority, and the root subagent_budget admits at most min(N, 8) calls across the whole recursive tree. For uncommitted workspace changes, use commit_message to author the complete Git message; Ducktape preserves it. Git commits you create keep their own messages. Omit commit_message when no uncommitted changes remain. Do not include markdown fences around the JSON."#;
