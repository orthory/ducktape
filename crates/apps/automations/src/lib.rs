//! deterministic user-defined automations over the chat hook seam.
//!
//! an operator registers rules — a [`Trigger`] (chat post filters) plus an
//! [`Action`] (post a chat message, or create a task). when chat fans a post out
//! to its hooks, this module evaluates every enabled rule and emits the matching
//! actions as follow-up [`sdk::Msg`]s in the SAME block as the post (P2).
//!
    ...
    fn build_and_emit(
        ...
        match &rule.action {
            Action::PostMessage {
                ...
                let body = substitute(template, event_channel, seq, author_display, text);
                if body.is_empty() {
            ...
            Action::CreateTask {
                ...
                let title = substitute(title_template, event_channel, seq, author_display, text);
                if title.is_empty() {
...
/// placeholders `{channel}`, `{seq}`, `{author}`, `{text}` are replaced from the
/// triggering event; any other `{...}` token is left literal. a single pass
/// means substituted values are never re-scanned for placeholders.
