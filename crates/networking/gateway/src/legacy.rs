//! FROZEN routes-only gateway module for the native protocol-v1 registry.
//!
//! This is the pre-merge `Gateway` module verbatim: it owns ONLY the route
//! plane and produces the ORIGINAL routes-only root/snapshot encoding. The
//! `.duck` handle plane was a SEPARATE `duckdns` module on that network, so the
//! v1 checkpoint carries a routes-only `gateway` snapshot AND a separate
//! `duckdns` snapshot — both of which the frozen native-v1 restore
//! (`bin/node/src/host_state/native_v1.rs`) reconstructs byte-for-byte.
//!
//! Nothing new should be built on this. The live path is the merged
//! [`crate::Gateway`], which owns the whole name → AccountId → route pipeline.
...
impl Module for LegacyGateway {
    ...
    async fn execute(&mut self, ctx: &mut dyn Ctx, msg: &Msg) -> Result<(), Error> {
        ...
        match decode_msg(&msg.payload).map_err(Error::Module)? {
            ...
            // the routes-only v1 module never carried the handle plane — those
            // ops rode the separate `duckdns` module on the v1 network.
            GatewayMsg::SetHandle { .. } => Err(Error::Module(
                "gateway: the native-v1 routes module does not accept handle ops".into(),
            )),
        }
    }
    ...
    async fn query(&self, req: &[u8]) -> Result<Vec<u8>, Error> {
        match decode_query(req).map_err(Error::Module)? {
            ...
            GatewayQuery::Resolve { .. } | GatewayQuery::Registrations { .. } => Err(Error::Module(
                "gateway: the native-v1 routes module does not serve handle queries".into(),
            )),
        }
    }
