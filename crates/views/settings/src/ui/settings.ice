...
// lifecycle, as the desktop app's own screen (screens/settings.ice before the
// port). No app state is reachable from here: every reading is a prop the app
// pushed, the drafts are the view's own, and every act leaves as an intent
// (app.ice) the app's handler of the same name signs. Node operations have
// their own rail surface; the workspace card links there instead of
// duplicating its facts here.

// WHICH GROUP IS ON SCREEN — the strip's own, never the app's.
enum SettingsPane
  general
  network
  account
                ...
                row
                  ...
                  col w=fill gap=3.0
                    ...
                    if desktop_notifications
                      text "A desktop banner when you are named, or written to directly."
                        with
                          size=11.0
                          @text-caption
                    if !desktop_notifications
                ...
                row
                  ...
                  col
                    ...
                    row
                      ...
                      if admin
                        Badge.Secondary label=tier
                      if !admin && !empty(tier)
                        Badge.Outline label=tier
                      if empty(tier) && members_answered
