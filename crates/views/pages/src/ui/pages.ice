    ...
    row w=fill h=fill
      col w=fill h=fill
        ...
        stack
          ...
          // COMMENTS FLOAT OVER THE DOCUMENT; THEY DO NOT DOCK BESIDE IT. A
          // rail took its width off the document for as long as it stayed
          // open, so pressing a count reflowed every line the reader was
          // reading. This card drops over the right edge of the page instead
          // and the document keeps its width and its place underneath.
          //
          // DECLARED AFTER THE DOCUMENT ARM, for the same reason as the two
          // search floats above — a stack draws its layers in declaration
          // order, first at the BOTTOM, so declared earlier the card would
          // paint under the text it covers and the editor would own the
          // pointer straight through it.
          //
          // NOT AN `overlay`: an overlay layer rides a backdrop that eats
          // every press outside it, and comments are read WHILE the document
          // is read and typed in. The page underneath stays live and only the
          // × closes the card — which is also why `escape_target` gives it no
          // rung, the compose field holds a half-typed draft.
          //
          // 340 FIXED, not a fraction: ice has no `max-w=fill`, and the
          // console never gets narrow enough to want one — `min-size 1040`
          // leaves this pane 734px, and the `clip=true` stack is the backstop.
