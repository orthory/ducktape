...
/// The PATH form (`/.duck/<authority>/<path>`) is the same door as the header
/// form: one resolver, one proxy. Same authority and path in, the SAME proxy
/// head out — only the page origin differs, because the app's OS browser has
/// no `duck://` origin to send.
#[tokio::test]
async fn gateway_browser_path_form_proxies_the_same_head_as_the_header_form() {
    let listen = "127.0.0.1:49152";
    let mut heads = Vec::new();
    for uri in ["/api/items?page=2", "/.duck/app.demo.duck/api/items?page=2"] {
        let (handle, cmds, _events) = local_node();
        spawn_duck_actor(cmds, 3);
        let (lane, mut jobs) = tokio::sync::mpsc::channel::<noded::GatewayJob>(1);
        let handle = handle
            .with_gateway(lane)
            .with_browser_gateway(listen.parse().unwrap());
        let (head_tx, head_rx) = tokio::sync::oneshot::channel();
        tokio::spawn(async move {
            let noded::GatewayJob::Http { head, reply, .. } = jobs.recv().await.unwrap() else {
                panic!("expected an http gateway job");
            };
            ...
            let _ = reply.send(Ok(noded::GatewayResponse {
                ...
                body: {
                    let (tx, rx) = tokio::sync::mpsc::channel(1);
                    tx.try_send(Ok(bytes::Bytes::from_static(b"<h1>hi</h1>")))
                        .unwrap();
                    drop(tx);
                    rx
                },
        ...
        if !uri.starts_with("/.duck/") {
            request = request.header("x-duck-authority", "app.demo.duck");
        }
        let response = noded::gateway_browser_router(handle)
        ...
        heads.push((head_rx.await.unwrap(), response));
    }
    let (path_head, path_response) = heads.pop().unwrap();
    let (header_head, _) = heads.pop().unwrap();
    assert_eq!(path_head.account_id, header_head.account_id);
    assert_eq!(path_head.name, header_head.name);
    assert_eq!(path_head.revision, header_head.revision);
    assert_eq!(path_head.method, header_head.method);
    assert_eq!(path_head.path_and_query, header_head.path_and_query);
    assert_eq!(path_head.path_and_query, "/api/items?page=2");
    // The page origin is the listener's own, so the page's subresources are
    // not blocked by the CSP the header form writes for `duck://`.
    let origin = format!("http://{listen}");
    assert_eq!(
        path_response.headers()["access-control-allow-origin"],
        origin
    );
    assert!(
        path_response.headers()[header::CONTENT_SECURITY_POLICY]
            .to_str()
            .unwrap()
            .contains(&format!("script-src 'unsafe-inline' {origin}"))
    );
}
