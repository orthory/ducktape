...
fn main() {
    // ducktape patch: skip the C build entirely on wasm32. Every shipped guest
    // module dead-strips blst (no module calls bls12381), and compiling C for
    // wasm needs a wasm-capable clang that neither stock linux gcc nor apple
    // clang provides. If a guest ever really calls blst, wasm-ld fails loudly
    // on the missing symbols. See PATCH.md.
