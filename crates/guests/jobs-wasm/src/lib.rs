//! `jobs-wasm` — the wasm port of the `jobs` module, built the ADAPTER way:
//! the NATIVE `jobs` crate is compiled to wasm32 unmodified and adapted to
...
//!    per-op staged overlays (job board AND worker set) into their committed
//!    maps — and
...
//!   snapshot — exactly like the native module's overlay-aware `get` reading
//!   its staged writes (a second claim in one block observes the first claim's
//!   staged `Processing`). mid-block queries (ctx-less and host-routed alike)
//!   run over the same overlay (`wasm-host::query_round`), so they serve
//!   committed + this block's staged writes — byte-for-byte what the native
//!   `query` serves from committed state plus the published inner overlay.
...
//!   the overlays. roots move at commit, never inside a block.
...
//!   native `abort_block` clears the overlays. (batch replay after a rejected
...
//!   likewise leave the overlays untouched.
//!
...
fn loaded_module() -> Result<Jobs, host::Error> {
    let mut module = Jobs::new(MODULE_ID);
    if let Some((bytes, root)) = load_state() {
        module
            .install(&bytes, StateRoot(root))
            .map_err(|e| host::Error::Rejected(format!("jobs state reload: {e}")))?;
    }
    Ok(module)
}

/// map an inner sdk error onto the wit surface. `Module` is the native
/// rejection verbatim; anything else a native jobs never surfaces from
