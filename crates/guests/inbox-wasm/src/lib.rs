//! `inbox-wasm` — the wasm port of the `inbox` module, built the ADAPTER way:
...
fn loaded_module() -> Result<Inbox, host::Error> {
    let mut module = Inbox::new(MODULE_ID);
    if let Some((bytes, root)) = load_state() {
        module
            .install(&bytes, StateRoot(root))
            .map_err(|e| host::Error::Rejected(format!("inbox state reload: {e}")))?;
    }
    Ok(module)
}

/// map an inner sdk error onto the wit surface. `Module` is the native
/// rejection verbatim; anything else a native inbox never surfaces from
