//! `duckdns-wasm` — the wasm port of the `duckdns` module, built the ADAPTER
//! way: the NATIVE `duckdns` crate is compiled to wasm32 unmodified and adapted
...
//! 2. runs the native `execute` over a [`WitCtx`] — INCLUDING its two
//!    cross-module gates: the valset standing check (validators ∪ residents)
//!    and the identity `OfNode` account derivation, both host-routed
...
//!    registry's per-op `pending` overlay into its committed map — and
...
//!   snapshot — exactly like the native registry's `effective()` reading its
//!   `pending` overlay (a second `SetHandle` in one block observes the first
//!   one's staged rename). mid-block queries run over the same overlay
//!   (`wasm-host::query_round`), so they serve committed + this block's staged
//!   writes — byte-for-byte what the native `query` serves from committed +
//!   `pending`.
...
//! reserved keys — a STATE-SCHEMA BREAK versus the native root (revision 2;
...
/// that derives the authenticated AccountId from the submitting node, and the
/// valset module that gates namespace mutations to members.
...
fn loaded_module() -> Result<DuckDns, host::Error> {
    let mut module = DuckDns::new(MODULE_ID, IDENTITY_ID, Some(VALSET_ID.into()));
    if let Some((bytes, root)) = load_state() {
        module
            .install(&bytes, StateRoot(root))
            .map_err(|e| host::Error::Rejected(format!("duckdns state reload: {e}")))?;
    }
    Ok(module)
}

/// map an inner sdk error onto the wit surface. `Module` is the native
/// rejection verbatim; anything else a native duckdns never surfaces from
...
impl Guest for Component {
    ...
    fn query(req: Vec<u8>) -> Result<Vec<u8>, host::Error> {
        ...
        // folded into `__state`. duckdns queries are pure registry reads (no
        // sibling access), so the ctx-less native `query` is the whole surface.
