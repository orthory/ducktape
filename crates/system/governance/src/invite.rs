...
//!
//! minting IS the admission decision FOR THAT KEY: a token is the issuer's
//! ed25519 signature over `binding ‖ nonce ‖ target ‖ role ‖ expiry`, where
//! `binding` is the network's genesis namespace (chain-id + genesis
//! fingerprint), `nonce` is per-invite randomness, `target` is the ONE key
//! the invite admits (no bearer invites), `role` selects the standing plane
//! (`Resident` now, `Client` reserved for the thin-client plane), and
//! `expires` is the unix-seconds redemption deadline. the joiner proves
//! possession of its own announced key by signing `binding ‖ nonce ‖ joiner`
//! — a blob holder cannot redeem under a key that never asked to join, and a
//! non-target key is refused even with a valid self-proof. redemption is
//! SINGLE-USE, enforced in consensus state by this crate's `GovMsg::Redeem`
//! handler (the nonce is the exactly-once key).
//!
...
/// `sign(INVITE_GRANT_NAMESPACE, binding ‖ nonce ‖ target ‖ role ‖ expiry)`.
...
/// the standing an invite grants its target. `Resident` is the only role
/// redeemable this generation; `Client` exists ON THE WIRE so the thin-client
/// plane needs no second invite-format change — a `Client` token is signed and
/// carried like any other but REJECTED at redeem until that plane lands.
...
pub struct InviteToken {
    ...
    pub role: InviteRole,
    /// unix seconds; enforced at decode (joiner) and in-consensus (redeem vs
    /// consensus_time).
    pub expires_unix_secs: u64,
    /// issuer's signature over `binding ‖ nonce ‖ target ‖ role ‖ expiry` in
    /// the invite-grant namespace.
    pub sig: ed25519::Signature,
}

/// the signed preimage of an invite grant: `binding ‖ nonce ‖ target ‖ role ‖
...
/// verify a token: issuer signature over `binding ‖ nonce ‖ target ‖ role ‖
/// expiry`.
...
mod tests {
    ...
    fn a_client_role_survives_the_signature_and_role_bytes_round_trip() {
        ...
        let msg = grant_preimage(BINDING, &nonce, &target.public_key(), role, expires);
