//! `capability-wasm` — the wasm port of the `capability` module, built the
//! ADAPTER way: the NATIVE `capability` crate is compiled to wasm32 unmodified
...
//!
//! the registry is MEMBER-GATED: every announce first queries the valset
//! module's live Validators and Residents projections (`ctx.query("valset",
//! ..)`) and rejects a submitter with no current standing. inside the guest
//! those are host imports the sync world cannot await, so the runtime resolves
//! them by MEMOIZED REPLAY — the run pauses on an unanswered read, the host
//! resolves it through the real `Ctx`, and the pure guest re-runs with the
//! answer memoized. an op's ACCEPTANCE therefore depends on sibling queries
//! resolving through the wasm runtime, which is exactly what the parity proof
//! pins.
//!
...
//!    per-op staged replacements into the committed registry — and
...
//!   snapshot — exactly like the native module's `effective()` reading its
//!   pending overlay (two nodes announcing in one block both land; the later
//!   save carries the earlier entry).
...
/// `CapabilityRegistry::new("capability", Some("valset".into()))`. the valset
/// id is genesis config, not committed state, so the guest must gate against
/// the same sibling as every native host — drift here would be a consensus
/// fork (an ungated guest would admit announces the native module rejects).
fn loaded_module() -> Result<CapabilityRegistry, host::Error> {
    let mut module = CapabilityRegistry::new(MODULE_ID, Some("valset".into()));
    if let Some((bytes, root)) = load_state() {
        module
            .install(&bytes, StateRoot(root))
            .map_err(|e| host::Error::Rejected(format!("capability state reload: {e}")))?;
    }
    Ok(module)
}

/// map an inner sdk error onto the wit surface. `Module` is the native
/// rejection verbatim; anything else a native capability never surfaces from
...
impl Guest for Component {
    ...
    fn query(req: Vec<u8>) -> Result<Vec<u8>, host::Error> {
        ...
        // query's committed+pending projection serves it with an empty pending
        // — the live (staged-overlay) view the runtime hands this round is
        // already folded into `__state`.
