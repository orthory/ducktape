 ...
 * continuation; a host::worker claims the effect and runs the messy work
 * off-path (each binary drives the loop); the result re-enters as an ordinary
 * ordered op that every validator replays identically.
 */
...
const nodes: SemanticNode[] = [
  ...
  { id: 'worker', label: 'host::worker', sub: 'worker seam, off-path', category: 'kernel' },
...
const edges: SemanticEdge[] = [
  ...
  { from: 'saga', to: 'worker', label: 'offers', kind: 'effect' },
  { from: 'worker', to: 'world', label: 'performs' },
...
const legend = [
  ...
  { color: '#8b93f0', label: 'worker (off-path)' },
...
export function AsyncEngine({ height = 340 }: { height?: number }) {
  return (
    <GraphFlow
      ...
      description="Modules never do non-deterministic work inline. They emit an Effect; saga records a deterministic continuation and a host::worker performs the messy work off the hot path (each binary drives its own loop); the result re-enters as an ordinary ordered op that every validator replays to the same state."
      nodes={nodes}
      edges={edges}
      legend={legend}
