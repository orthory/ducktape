...
Date: 2026-07-06 · Status: **Built** (2026-07-07, `feat/video-calls`).
Structural deviations from the accepted direction: the webview↔node leg is the huddle build's
...
and media initially rode the authenticated TCP mesh arm rather than waiting on
the slice-0 overlay socket. **Update 2026-07-09:** media has since moved to the
overlay-socket arm — one per-use plane per service — because the shared mesh
relay queue starved voice behind video under load; see the 2026-07-09 update in
...
- **Trust boundary matches private messaging**
  (`docs/adr/2026-07-06-private-team-messaging.mdx`): media is point-to-point
...
The complete draft this ADR condensed (evidence dossier with sources, layer
diagram, failure modes, test plan, code anchors) is preserved in git history:
added in 5ae8235, removed in the commit that adds this ADR (branch
`feat/video-call-design`).
