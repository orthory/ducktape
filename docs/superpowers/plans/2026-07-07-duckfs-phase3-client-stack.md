...
**Files:**
- Modify: `docs/src/content/docs/en/human/modules/product-modules.mdx` — the duckfs section gains the client story: `ducktape-fs` verbs, `.duckfs` index, conflict semantics (auto-rebase disjoint only), workspace RPC; note `mount` is Phase 4.
- Modify: `docs/src/content/docs/en/agent/reference/repository-map.mdx` — `crates/system/duckfs-client`, `bin/fs`.
