...
The node is supervisor-ready: SIGTERM takes the graceful checkpoint path
(the same one the desktop shell uses on quit), it raises its own open-file
...
Firecracker, `mke2fs`, `debugfs` and (for a tap-networked run) `nft` must be
on the service's `PATH` — `/usr/local/bin`, `/usr/sbin` and `/sbin` are
searched (`crates/services/sandbox/src/host_tools.rs`).
