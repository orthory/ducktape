...
| `keys/<name>.key` + `keys/active` | `ducktape wallet new` / `wallet import` (`crates/keystore/src/wallet.rs`; argon2id + XChaCha20-Poly1305 at rest, 0600, `create_new`) | **yes** (encrypted) | **Your user identity** — the key your account's ops are signed with. The 24-word mnemonic is the backup: `ducktape wallet import` reproduces the same key. Lose both and the account is unreachable until another of its keys adds a new one (`ducktape account key add`); an account with one key is gone. `active` is a one-line pointer, regenerable with `ducktape wallet use`. |
