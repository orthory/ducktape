...
| Forge | PORT (local git2 read service) | `views/forge/CodeBrowser + ForgeList` + reference `crates/forge` read side | See §5. **Reads** (repo/log/tree/file/diff) via a local Tauri git2 service opening the node's on-disk repo — ports the reference's `forge-client`/CodeBrowser almost directly (shapes match). **Writes** (commit) stay on the node consensus wire (`ForgeMsg::Commit`, already wired). Single branch `main` (node's model). **No PRs/issues** (not lightweight). |
