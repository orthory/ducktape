...
The Approvals tab is not native: it is `crates/views/governance`, an Ice
application compiled for the `tree` target and wrapped as an `ice:view`
component that the app loads from a file at runtime (`src/module_view.rs`).
...
`governance.execute`) that the tab's handler signs and submits exactly as the
native screen did. The guest sees no key, no endpoint and no clock, and a view
