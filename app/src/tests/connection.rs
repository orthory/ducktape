...
fn every_data_screen_answers_a_dead_node_with_not_connected() {
    ...
    /// down), Node (which owns the daemon diagnostics), Chat, Files and Pages
    /// are module-owned views now and not in this inventory; every native
    /// data screen answers.
    const EXEMPT: [&str; 0] = [];
