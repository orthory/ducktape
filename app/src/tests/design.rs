...
fn compact_controls_share_a_single_geometry_and_type_scale() {
    ...
    // Chat and pages both draw it, from their screens — pages' lives in
    // the `pages` view now, so both sources are swept together.
...
fn every_repeated_component_mount_is_culled_or_argued() {
    ...
    const ARGUED: &[(&str, &str)] = &[
        ...
        // A recovery phrase in twelve numbered pairs. Exactly 24 words, once,
