...
impl Guest {
    ...
    /// host-side (an input's text) and queued for the guest's next tick. The
    /// node view's surface event is the app's, not the guest's: it was
    /// queued where the ring keeps it, and the app is told to drain it. Any
    /// other surface's value — a link the forge reader opened — is the
    /// guest's route, and crosses like a widget's.
