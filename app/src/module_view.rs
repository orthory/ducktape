...
/// `-> unit` — hears only that something happened. The shell view's three:
/// the terminal for the session the app parked, the answer Markdown (a link
/// it opens goes back to the guest's handler), and the composer
/// (`crate::shell_composer`), whose submit is the app's `send` intent.
    ...
    if module == "shell" {
        ...
        surfaces.insert(
            ...
            Arc::new(|_key: &str, args: &[wire::SurfaceValue]| {
                ...
                crate::backend::agent_markdown(source.clone(), *dark).map(wire::SurfaceValue::Str)
            }),
        );
...
impl Guest {
    ...
    /// app's own — the node's ring, whose event was queued where the
    /// surface keeps it, or the shell composer's send — and becomes the
    /// app's intent.
    fn deliver(&mut self, output: Output) {
        ...
        } = output
        {
            match self.module {
                ...
                "shell" => self.intents.extend(crate::shell_composer::intent(&value)),
                _ => {}
            }
            return;
        }
