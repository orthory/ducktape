...
describe("accountUsage", () => {
  it("groups executors by resolved account with a per-capability breakdown", async () => {
    ...
    const wire = [
      row({ executorHex: "aa11", capability: "model-1", runs: 2, totalDurationBlocks: 9 }),
      row({
        executorHex: "aa11",
        capability: "model-1",
        outcomeOk: false,
      ...
      row({ executorHex: "bb22", capability: "codex", runs: 3, totalDurationBlocks: 12 }),
    ...
    expect(groups[0].byCapability).toEqual([
      { capability: "model-1", runs: 3, failed: 1, totalDurationBlocks: 13 },
      { capability: "codex", runs: 3, failed: 0, totalDurationBlocks: 12 },
    ]);
