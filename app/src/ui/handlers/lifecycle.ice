...
on select_shell_tab(next)
  ...
  // `moved` because a re-select of the screen she is on is not a move. The
  // chat menus are the VIEW's now and it retires its own on the same rule.
...
subscribe
  ...
  keyboard press key=escape status=captured when !empty(topmost_overlay(shell_tab, palette_open, bell_open, channel_create_open, page_delete_armed)) -> global_key_pressed _
