  ...
  col width=fill spacing=2.0
    if !editing && !empty(title)
      button label=title disabled=disabled width=fill padding=4.0 -> begin(title)
        ...
        hovered bg=white/4 text=fg border=white/6
        pressed bg=white/7 text=fg border=white/9
    if !editing && empty(title)
...
component BlockActionsMenu(block_id:str, kind:str, disabled:bool, delete_armed:bool, editable_kinds:[str])
  container width=172.0 padding=5.0 bg=popover border=white/13 border-w=1.0 r=10.0 shadow=black/24 shadow-y=4.0 shadow-blur=14.0
    col width=fill spacing=3.0
      if kind != "Page"
        pick editable_kinds some(kind) placeholder="Block type" width=fill menu-height=210.0 padding=6.0 text-size=11.0 line-height=1.2 -> selected_block_kind_changed _
          ...
          hovered text=fg placeholder=muted handle=fg bg=white/8 border=white/10 border-w=1.0 r=6.0
          opened text=fg placeholder=muted handle=fg bg=white/11 border=white/13 border-w=1.0 r=6.0
          menu text=fg selected-text=fg selected-bg=white/14 bg=popover border=white/13 border-w=1.0 r=8.0 shadow=black/18 shadow-y=3.0 shadow-blur=11.0
      if kind == "Page"
      ...
      if delete_armed
        button "Confirm delete" label="Confirm block deletion" disabled=disabled width=fill height=28.0 padding=6.0 -> remove_block_submit
          active bg=white/10 text=fg border=white/12 border-w=1.0 r=6.0
          hovered bg=white/14 text=fg border=white/15
          pressed bg=white/18 text=fg border=white/17
...
component InlineBlockInsert(kind:str, kinds:[str], disabled:bool, prefix:str)
  stack width=fill
    ...
    container width=fill align-x=end align-y=start padding-right=4.0
      container padding=2.0 bg=popover border=white/12 border-w=1.0 r=8.0 shadow=black/14 shadow-y=2.0 shadow-blur=7.0
