...
impl View {
    ...
    fn replace(&self, name: &str, args: &[wire::SurfaceValue], guest: &Guest,
        window: &mut Window, cx: &mut Context<NativeModuleView>) -> Result<(), String> {
        match self {
            ...
            Self::Picture(view) => {
                view.update(cx, |view, cx| view.replace(surface_str(args, 0), surface_str(args, 1), cx));
                Ok(())
            }
            Self::Code(view) => {
                view.update(cx, |view, cx| view.replace(surface_str(args, 0), surface_str(args, 1), surface_bool(args, 2), window, cx));
                Ok(())
            }
            Self::Asset(view) => {
...
fn markdown_args(name: &str, args: &[wire::SurfaceValue]) -> (String, String, bool) {
    match name {
        ...
        _ => (surface_str(args, 0), surface_str(args, 1), surface_bool(args, 2)),
    }
}

fn asset_node(name: &str, args: &[wire::SurfaceValue], guest: &Guest) -> wire::Node {
    let path = surface_str(args, 0);
    let Some(bytes) = artifact_asset(&guest.assets, &path) else { return wire::Node::empty(); };
    use sha2::{Digest as _, Sha256};
    let digest = Sha256::digest(bytes);
    let hash = u64::from_le_bytes(digest[..8].try_into().expect("hash prefix"));
    match name {
        "artifact_svg" => wire::Node::Svg {
            key: path, hash, bytes: Some(bytes.to_vec()), inherit_button_ink: false,
            label: None, color: None, hover: None, fit: None, rotation: None, opacity: None,
            width: Some(wire::Length::Fill), height: Some(wire::Length::Fill),
        },
        _ => wire::Node::Image {
            key: path, hash, data: Some(wire::ImageData::Encoded(bytes.to_vec())), label: None,
            fit: None, rotation: None, opacity: None, filter: Default::default(),
            width: Some(wire::Length::Fill), height: Some(wire::Length::Fill),
        },
    }
}
...
impl NativeModuleView {
    pub(super) fn sync_surfaces(&mut self, guest: &Guest, window: &mut Window,
        cx: &mut Context<Self>) -> Result<(), String> {
        let Some(content) = self.content.clone() else { return Ok(()); };
        let requests = content.read(cx).surface_requests();
        self.surfaces.retain(|key, _| requests.iter().any(|request| &request.0 == key));
        for (key, name, args, handler) in requests {
            if !surface_allowed(self.module, &name) { continue; }
            let same_kind = self.surfaces.get(&key).is_some_and(|surface| surface.name == name);
            if !same_kind {
                let view = match name.as_str() {
                    "chat_composer" | "forge_composer" => {
                        crate::composer_surface::validate_args(&args)?;
                        View::Composer(cx.new(|cx| crate::composer_surface::ComposerView::new(&args, window, cx)
                            .expect("validated composer arguments")))
                    }
                    "agent_markdown" | "forge_markdown" => {
                        let (source, doc, dark) = markdown_args(&name, &args);
                        View::Markdown(cx.new(|cx| crate::backend::MarkdownSurface::new(source, doc, dark, cx)))
                    }
                    "picture" => View::Picture(cx.new(|_| crate::backend::PictureView::new(surface_str(&args, 0), surface_str(&args, 1)))),
                    "forge_code" => View::Code(cx.new(|cx| crate::backend::CodeView::new(surface_str(&args, 0), surface_str(&args, 1), surface_bool(&args, 2), window, cx))),
                ...
                self.surfaces.insert(key, Surface { name, args, handler, view, _events: events });
                continue;
            }
    ...
    fn surface_subscription(&self, view: &View, key: &str, handler: Option<u32>,
        cx: &mut Context<Self>) -> Option<Subscription> {
        match view {
    ...
    fn subscribe_surface<V: EventEmitter<wire::SurfaceValue> + 'static>(&self,
        view: &Entity<V>, key: &str, handler: Option<u32>, cx: &mut Context<Self>) -> Subscription {
        ...
        cx.subscribe(view, move |this, _, value, cx| {
            let current_route = this.surfaces.get(&key).is_some_and(|surface| surface.handler == handler);
