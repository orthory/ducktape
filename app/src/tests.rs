...
fn semantic_recipes_own_action_focus_and_status_colors() {
    ...
    /// Latest Ice owns button-child ink: `color=inherit` reads the button's
    /// resolved active/hovered/pressed/disabled text color. The app should use
    /// that native channel, never restore the old component/style workaround.
    fn assert_icon_controls_inherit_ink(name: &str, source: &str, expected: usize) {
        assert_eq!(
            source.matches("color=inherit").count(),
            expected,
            "{name}: icon-only buttons must inherit their button ink"
        );
    }
    ...
    assert_icon_controls_inherit_ink("chat.ice", &chat, 3);
    assert_icon_controls_inherit_ink("screens/chat.ice", &chat_screen, 1);
