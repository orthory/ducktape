...
pub(crate) async fn folded_update(
    ...
    match module {
        ...
        // `pages` is one of them now: the pages view reads its own workspace
        // through the kernel and re-reads on the `rpc.live` hit this plane
        // update delivers, so the app folds nothing of its own for it.
        "valset" | "governance" | "identity" | "agent" | "runs" | "files" | "pages" => {
