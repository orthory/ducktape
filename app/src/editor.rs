...
//!
//! Selected mentions carry byte ranges and stable parties beside the editor
...
pub fn mention_query(document: &Content) -> Option<MentionQuery> {
    ...
    let partial_is_a_handle = partial
        .chars()
...
/// Encode only selected mention ranges; ordinary @text stays ordinary text.
pub fn mention_body(content: &Content, mentions: &[(Range<usize>, chat::Party)]) -> String {
...
/// Copy whole identities even when the visible selection clips a name.
pub fn copy_mention_selection(
    content: &mut Content,
    mentions: &[(Range<usize>, chat::Party)],
) -> Option<String> {
    content.selection().filter(|text| !text.is_empty())?;
    let deletion = ComposerEvent::Apply(RichAction::Edit(text_editor::Action::Edit(Edit::Delete)));
    select_mention_edit(content, mentions, &deletion);
    let cursor = content.cursor();
    let caret = text_offset(content, cursor.position);
    let anchor = text_offset(content, cursor.selection?);
    let selected = caret.min(anchor)..caret.max(anchor);
    let selected_mentions: Vec<_> = mentions
        .iter()
        .filter(|(range, _)| range.start >= selected.start && range.end <= selected.end)
        .map(|(range, party)| {
            (
    ...
    let text = content.text();
    Some(mention_body(
        &Content::with_text(&text[selected]),
        &selected_mentions,
    ))
}

/// Plain clipboard text carries IDs; hydrate only canonical tokens, never names.
pub fn paste_mentions(
    mut content: Content,
    mentions: &mut Vec<(Range<usize>, chat::Party)>,
    body: &str,
    names: &crate::backend::NameDirectory,
) -> Content {
    let body = body.replace("\r\n", "\n").replace('\r', "\n");
    let (text, pasted_mentions) = chat::client::draft_mentions(&body, names);
    let event = ComposerEvent::Apply(RichAction::Edit(text_editor::Action::Edit(Edit::Paste(
        std::sync::Arc::new(text.clone()),
    ))));
    select_mention_edit(&mut content, mentions, &event);
    let cursor = content.cursor();
    let caret = text_offset(&content, cursor.position);
    let anchor = cursor
        .selection
        .map_or(caret, |at| text_offset(&content, at));
    let selected = caret.min(anchor)..caret.max(anchor);
    let start = selected.start;
    replace_mention_ranges(mentions, selected, text.len());
    mentions.extend(
        pasted_mentions
            .into_iter()
            .map(|(range, party)| (start + range.start..start + range.end, party)),
    );
    mentions.sort_by_key(|(range, _)| range.start);
    apply_composer_event(content, event)
}

/// Move ranges using the editor's actual replacement, including repeated labels.
fn replace_mention_ranges(
    mentions: &mut Vec<(Range<usize>, chat::Party)>,
...
pub fn apply_mention_event(
    mut content: Content,
    mentions: &mut Vec<(Range<usize>, chat::Party)>,
    event: ComposerEvent,
) -> Content {
    if let ComposerEvent::Mark(kind) = event {
        return toggle_mention_mark(content, mentions, kind);
    }
    select_mention_edit(&mut content, mentions, &event);
    ...
    let anchor = cursor
        .selection
        .map_or(caret, |at| text_offset(&content, at));
    let mut replaced = caret.min(anchor)..caret.max(anchor);
    let next = apply_composer_event(content, event.clone());
    let after = next.text();
    if before == after {
        return next;
    }
    match event {
        ComposerEvent::Apply(RichAction::Edit(text_editor::Action::Edit(Edit::Backspace)))
            if replaced.is_empty() =>
        {
            replaced.start = caret - (before.len() - after.len());
        }
        ComposerEvent::Apply(RichAction::Edit(text_editor::Action::Edit(Edit::Delete)))
            if replaced.is_empty() =>
        {
            replaced.end = caret + (before.len() - after.len());
        }
    ...
    let inserted = after.len() - (before.len() - replaced.len());
    replace_mention_ranges(mentions, replaced, inserted);
    next
}

pub fn toggle_mention_mark(
    mut content: Content,
    mentions: &mut Vec<(Range<usize>, chat::Party)>,
    kind: String,
) -> Content {
    ...
    for (range, _) in mentions.iter() {
        let intersects = start < range.end && end > range.start;
        let inside = start == end && start > range.start && start < range.end;
    ...
    content.move_to(Cursor {
        position: text_position(&before, end),
        selection: Some(text_position(&before, start)),
    });
    let next = composer_toggle_mark(content, kind);
    replace_mention_ranges(mentions, end..end, close.len());
    replace_mention_ranges(mentions, start..start, open.len());
    next
}

fn text_offset(content: &Content, at: Position) -> usize {
    (0..at.line)
        .filter_map(|line| content.line(line))
        .map(|line| line.text.len() + 1)
        .sum::<usize>()
        + at.column
}

fn text_position(text: &str, offset: usize) -> Position {
...
/// A deletion or replacement through a token selects its entire visible name.
pub fn select_mention_edit(
    content: &mut Content,
    mentions: &[(Range<usize>, chat::Party)],
    event: &ComposerEvent,
) {
    let ComposerEvent::Apply(RichAction::Edit(text_editor::Action::Edit(edit))) = event else {
        return;
    };
    let cursor = content.cursor();
    let caret = text_offset(content, cursor.position);
    ...
    for (range, _) in mentions {
        let overlaps = start < range.end && end > range.start;
        let inside = start == end && start > range.start && start < range.end;
        let backspace = start == end && start == range.end && matches!(edit, Edit::Backspace);
        let delete = start == end && start == range.start && matches!(edit, Edit::Delete);
        if overlaps || inside || backspace || delete {
            start = start.min(range.start);
            end = end.max(range.end);
        }
    }
    if start == caret.min(anchor) && end == caret.max(anchor) {
        return;
    }
    let text = content.text();
    content.move_to(Cursor {
        position: text_position(&text, end),
        selection: Some(text_position(&text, start)),
    });
}
...
fn composer_chord(press: &KeyPress) -> Option<ComposerEvent> {
    ...
    let mark = match press.physical_key {
        Physical::Code(Code::KeyC) if !shifted => return Some(ComposerEvent::Copy),
        Physical::Code(Code::KeyX) if !shifted => return Some(ComposerEvent::Cut),
...
fn composer_key_binding(press: &KeyPress, menu_open: bool) -> Option<Binding<Edit>> {
    let owned_chord = matches!(
        composer_chord(press),
        Some(ComposerEvent::Copy | ComposerEvent::Cut)
    );
    if menu_key(press, menu_open).is_some() || owned_chord {
        return None;
    }
...
fn composer_format(kind: &ComposerHighlight) -> Format {
    match kind {
        ComposerHighlight::Markdown(inline) => inline_format(inline),
        ComposerHighlight::Mention => Format {
            color: Some(rgb8(0xa0, 0x5a, 0x3c)),
            font: Some(composer_font(Weight::Medium, FontStyle::Normal)),
            ..Format::default()
        },
    }
}
...
struct InlineMarkdownHighlighter {
    current_line: usize,
    mentions: Vec<Vec<Range<usize>>>,
}

impl Highlighter for InlineMarkdownHighlighter {
    type Settings = Vec<Vec<Range<usize>>>;
    type Highlight = ComposerHighlight;
    type Iterator<'a> = std::vec::IntoIter<(Range<usize>, ComposerHighlight)>;

    fn new(settings: &Self::Settings) -> Self {
        Self {
            current_line: 0,
            mentions: settings.clone(),
        }
    }

    fn update(&mut self, settings: &Self::Settings) {
        self.current_line = 0;
        self.mentions = settings.clone();
    }
    ...
    fn highlight_line(&mut self, line: &str) -> Self::Iterator<'_> {
        ...
        for (range, kind) in inline_marks(line) {
            let mut start = range.start;
            for mention in &mentions {
                let overlaps = mention.start < range.end && mention.end > start;
                if !overlaps {
                    continue;
                }
                if start < mention.start {
                    spans.push((start..mention.start, ComposerHighlight::Markdown(kind)));
                }
                start = mention.end.min(range.end);
            }
            if start < range.end {
                spans.push((start..range.end, ComposerHighlight::Markdown(kind)));
            }
        }
        spans.extend(
            mentions
                .into_iter()
                .map(|range| (range, ComposerHighlight::Mention)),
        );
...
mod tests {
    ...
    fn mark_chords_follow_slacks_table_at_the_widget() {
        ...
        assert_eq!(
            chord(Code::KeyC, Modifiers::COMMAND),
            Some(ComposerEvent::Copy)
        );
        assert_eq!(
            chord(Code::KeyX, Modifiers::COMMAND),
            Some(ComposerEvent::Cut)
        );
    ...
    #[test]
    fn mention_ranges_follow_repeated_labels_and_formatting() {
        let mut mentions = vec![
            (0..5, chat::Party::Account(5)),
            (6..11, chat::Party::Account(6)),
        ];
        let content = Content::with_text("@Same @Same");
        let mut content = apply_mention_event(
            content,
            &mut mentions,
            ComposerEvent::Apply(RichAction::Edit(text_editor::Action::Edit(Edit::Paste(
                std::sync::Arc::new("@Same ".into()),
            )))),
        );
        assert_eq!(mention_body(&content, &mentions), "@Same <@5> <@6>");
        content.perform(text_editor::Action::SelectAll);
        let content = toggle_mention_mark(content, &mut mentions, "bold".into());
        assert_eq!(mention_body(&content, &mentions), "**@Same <@5> <@6>**");
    }

    #[test]
    fn multiline_clipboard_paste_hydrates_ids_and_plain_replacement_removes_them() {
        let names = crate::backend::NameDirectory::default();
        let mut mentions = Vec::new();
        let mut content =
            paste_mentions(Content::new(), &mut mentions, "안녕\r\n<@5> <@6>", &names);
        assert_eq!(content.text(), "안녕\n@account-5 @account-6");
        ...
        assert_eq!(
            copy_mention_selection(&mut content, &mentions).as_deref(),
            Some("<@5>")
        );
    ...
    #[test]
    fn mention_highlighting_keeps_surrounding_markdown() {
        let mut highlighter = InlineMarkdownHighlighter::new(&vec![vec![8..13]]);
        let spans: Vec<_> = highlighter
            .highlight_line("**hello @Same world**")
            .collect();
        assert!(spans.iter().any(|(range, kind)| *range == (2..8)
            && matches!(kind, ComposerHighlight::Markdown(Inline::Bold))));
        assert!(spans.iter().any(|(range, kind)| *range == (13..19)
            && matches!(kind, ComposerHighlight::Markdown(Inline::Bold))));
        assert!(
            spans.iter().any(
                |(range, kind)| *range == (8..13) && matches!(kind, ComposerHighlight::Mention)
            )
        );
    }
}
