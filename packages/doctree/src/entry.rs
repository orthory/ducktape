...
pub enum Entry {
    /// A placeholder for entries the driver chose to skip (e.g. dotfiles) or
    /// that didn't resolve to anything. Surfaces during the build but isn't
    ...
    /// A directory's children, paired with their absolute paths. Each child
    /// sits behind an `Arc` so cloning a `Directory` only reallocates the
    ...
    /// previous version.
    Directory(Vec<(/*absolute*/ String, Arc<Entry>)>),
}
