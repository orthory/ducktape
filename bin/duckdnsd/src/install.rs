...
mod platform {
    ...
    pub(super) fn apply(id: &str, root_cert: &Path) -> Result<(), String> {
        let service = format!(
            "# {}\n[Unit]\nDescription=Ducktape DuckDNS device helper\nAfter=network.target systemd-resolved.service\nWants=systemd-resolved.service\n\n[Service]\nType=simple\nExecStart={BINARY} serve\nExecStartPost=/usr/bin/resolvectl dns lo 127.77.0.1\nExecStartPost=/usr/bin/resolvectl domain lo ~ducktape.quack\nRestart=on-failure\nRestartSec=2\nNoNewPrivileges=true\nPrivateTmp=true\nPrivateDevices=true\nProtectSystem=strict\nProtectHome=true\nReadWritePaths={}\nRestrictAddressFamilies=AF_INET AF_INET6 AF_UNIX\nCapabilityBoundingSet=CAP_NET_BIND_SERVICE\nAmbientCapabilities=CAP_NET_BIND_SERVICE\n\n[Install]\nWantedBy=multi-user.target\n",
            marker(id),
    ...
    pub(super) fn inspect(id: &str, problems: &mut Vec<String>) {
        ...
        match trust_anchor(id) {
            Ok(path) => inspect_owned(&path, id, problems),
            Err(error) => problems.push(error),
        }
        if !matches!(
