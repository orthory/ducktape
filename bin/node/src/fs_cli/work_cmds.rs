...
pub fn checkout(args: &[String]) -> Result<(), CliError> {
    let (pos, flags) = parse_flags(args)?;
    let [prefix, dir] = pos.as_slice() else {
        return Err(CliError::usage("checkout needs <prefix> <dir>"));
    };
    // a fresh checkout has no index yet, so the node MUST be explicit.
    let url = resolve_node(&flags)?;
    ...
    let index =
        checkout_with(&node, Path::new(dir), prefix, snapshot, &opts).map_err(checkout_err)?;
    let base = index.base_snapshot.as_deref().unwrap_or("(empty tree)");
    println!("checked out {prefix} at {base} into {dir}");
    Ok(())
}
...
pub fn status(args: &[String]) -> Result<(), CliError> {
    let (pos, _flags) = parse_flags(args)?;
...
pub fn commit(args: &[String]) -> Result<(), CliError> {
    let (pos, flags) = parse_flags(args)?;
    let dir = pos.first().map(String::as_str).unwrap_or(".");
    ...
    let message = flags
        .get("message")
        .filter(|m| !m.is_empty())
        .ok_or_else(|| CliError::usage("commit needs --message <m>"))?;
    let node = node_for_dir(&flags, dirp)?;
    let opts = CommitOptions {
        auto_rebase: !flags.contains_key("no-rebase"),
    };

    match commit_with(&node, dirp, message, &opts) {
        Ok(summary) => {
...
pub fn pin(args: &[String]) -> Result<(), CliError> {
    use duckfs_client::api::{ApiError, NodeApi};

    let (pos, flags) = parse_flags(args)?;
    let [snapshot, name] = pos.as_slice() else {
        return Err(CliError::usage("pin needs <snapshot> <name>"));
    };
    // pin runs against a node directly (default `.` so a checkout's index can
    // supply the node, but `--node`/env win).
    let node = node_for_dir(&flags, Path::new("."))?;
    node.pin(snapshot, name).map_err(|e| match e {
    ...
    println!("pinned {snapshot} as {name}");
    Ok(())
}
