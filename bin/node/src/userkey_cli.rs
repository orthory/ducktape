...
/// the LAST stdout line as the value), provenance on stderr. the legacy v1
/// shape (#205); `key init` is the v2 replacement `cmd_user_key` dispatches to
/// instead. `out` defaults to `user.key` when the bare form omits it.
