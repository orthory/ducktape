//! The validator role: consensus boot, mesh wiring, engine spawn, and the
//! main drain loop — run_node's fall-through path when the checkpoint (or
//! genesis set) seats this key. Moved verbatim from `run_node` (phases
//! P8–P14); the boot itself (P7/P11) lives in [`super::boot`].
...
use commonware_runtime::{Clock, IoBuf, Metrics, Quota, Spawner, Supervisor};
use commonware_utils::ordered::Set;
use futures::{FutureExt as _, StreamExt as _};

use crate::config::{self, hex_bytes, unhex};
use crate::constants::*;
use crate::explorer::{IndexFold, explorer_root_op, heal_index, ship_index_blobs};
...
use crate::rpc::{
    JoinRequestRecord, JoinRequestView, RpcJob, RpcReply, RpcRequest, RpcStatus,
    spawn_rpc_listener,
};
use crate::statesync_plane;
use crate::{blob_fetch, lobby, oracle_pool, relay, relay_runtime, validator};
use crate::sync::catchup::derive_pending_boot;
use crate::sync::serve::{SyncBoundary, SyncStateRequest, drive_sync_request};
use crate::util::{diag_log, epoch_floor, hex, participant_bytes, resident_bytes, unix_ms};
use crate::validator::announce::{
    CapabilityAnnouncer, ReadinessSignaller, dispatch_pending_deliveries, saga_next_expiry,
};
use crate::voice;
use crate::voice_plane;
...
pub(crate) async fn run_validator(
    ...
    let (host, resumed, next_seq, prev_ckpt, recovery_manifest_for_resume) =
        validator::boot::boot(
            &context,
            &label,
            &mut recovery,
    ...
    let (host, resumed, mut next_seq, mut prev_ckpt, recovery_manifest_for_resume, sync_tx, sync_rx) =
        validator::boot::post_reboot_catchup(
            promoted_validator_boot,
            &context,
            &label,
            &signer,
            &namespace,
